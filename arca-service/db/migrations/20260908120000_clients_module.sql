...
-- Ported from kyra-ipm-v4/supabase/migrations/20260908120000_clients_module.sql:
--   * FKs point at public.users instead of auth.users
--   * auth.uid() -> private.current_actor()
--   * dropped RLS / policies / grants
...
-- Down Migration
drop function if exists public.save_tax_category(bigint, text, numeric, boolean);
drop function if exists public.save_fiscal_condition(bigint, bigint, text, boolean);
drop function if exists public.save_country(bigint, text, text, boolean);
drop function if exists public.set_client_status(bigint, text, timestamptz);
drop function if exists public.save_client(jsonb, jsonb, timestamptz);
drop table if exists public.client_emails;
drop table if exists public.clients;
drop table if exists public.tax_categories;
drop table if exists public.fiscal_conditions;
